...
def save_all_transcripts():
    for libro in list_of_dicts:
        for k,v in libro.items():
            with open(k, 'w') as archivo:
                archivo.write(v)
    print('all transcripts are in text files in that one folder')

test_transcript = page1_transcripts['02x12 - The Mountain King']
good = process_transcript(test_transcript)
big_string = ' '.join(good).replace("\'", '').replace("\n",'')
test_string = big_string.translate(str.maketrans('','', string.punctuation)).lower()
bag_of_words = test_string.split(' ')

good_words = [word for word in bag_of_words if word not in stops]

la_cuenta = Counter(good_words)
la_cuenta.most_common()
'''